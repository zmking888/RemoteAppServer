...
namespace RemoteApp
{
	class WtsApi32
	{
		...
		[DllImport("Wtsapi32.dll", SetLastError = true)]
		//public static extern bool WTSVirtualChannelRead(IntPtr channelHandle, int TimeOut,byte[] data, int length, out int bytesRead);
        public static extern int WTSVirtualChannelRead(IntPtr hChannel,uint Timeout,[Out, MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 3)] byte[] Buffer, uint BufferSize,out uint BytesRead);
